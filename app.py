...
def test_email():
    ok = send_email(
        ...
        body="If you received this, SMTP is fully integrated 🎉"
    )
...
def reset_password():
    ...
    record = (
        ...
        .filter_by(email=email, used=True)
    ...
    # ✅ Set new password (argon2)
    user.set_password(new_password)

    # ✅ Invalidate ALL OTPs for this email (security)
    PasswordResetOTP.query.filter_by(email=email).update({"used": True})
    ...
    # ✅ Set JWT cookie (THIS IS CRITICAL)
    response = jsonify({
...
def upsert_link_for_long_url(
    ...
    # Vanity slug path
    if vanity_slug:
        if Link.query.filter(or_(Link.slug == vanity_slug, Link.short == vanity_slug)).first():
            return None, "Vanity slug already taken"
    ...
    # ✅ Reuse ONLY if same user
    if owner_id:
        existing = (
            Link.query
            .filter_by(long=long_url, owner_id=owner_id)
            .order_by(Link.created_at.desc())
            .first()
        )
...
def api_shorten():
    ...
    # ✅ SLUG VALIDATION (SAFE)
    if vanity_slug:
        if not re.fullmatch(r"[a-zA-Z0-9_-]{3,30}", vanity_slug):
            return jsonify({
                "error": "Slug must be 3–30 chars and contain only letters, numbers, - or _"
            }), 400
    ...
    link, err = upsert_link_for_long_url(
        long_url,
        expires_at=expires_at,
...
@admin_required
def admin():
    admin_user = db.session.get(User, int(get_jwt_identity()))
    if not admin_user:
        abort(404)
    ...
    return render_template(
        ...
        users=users,
        links=links,
        current_user=admin_user   # ✅ GUARANTEED
    )
...
def signup():
    ...
    set_access_cookies(response, access_token)   # ✅ THIS LINE FIXES IT
    return response, 201
...
def login():
    ...
    # 🔑 Claim unowned links
    Link.query.filter_by(owner_id=None).update(
        {"owner_id": user.id}
    )
    db.session.commit()
